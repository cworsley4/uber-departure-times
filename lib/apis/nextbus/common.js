      ...
      if (!cache) {
        ...
        return yield request.get(config.nextbus)
          ...
          .then(function (result) {
            // The NextBus api will return a 200 status code even if there is 
            // an error in the response. So we have to parse that to xml, check
            // that there isn't an error property before saving to the cache.
            
            // var data = xml2json(result.text).body;
            //debug(data);
            //if (!data.error) {
